...
def model_A(M):   # аллометрическая: 276 × (M/36.5)^0.25 эф. дн (калибрована по H.e. §8.4)
    return 276.0 * (M / 36.5) ** 0.25
# Бит-в-бит с уже посчитанной model_A_ef_days в GEOGRAPHY_ETERIA.json:
for nm, g in GEST.items():
    chk(f'C-G {nm}: A={g["model_A_ef_days"]} эф. дн', round(model_A(g['M_kg']), 1), g['model_A_ef_days'], 0.6)
...
VERDICT_GEST = dict(
    принят='МОДЕЛЬ A — аллометрическая: гестация = 276·(M/36.5)^0.25 ЭФИРИАНСКИХ суток '
           '(земный рендер ×7/9: H.e. 276 эф. дн = 214.7 зем. дн = 7.05→«7.1» мес карточки — §8.4)',
    ...
    rationale=['A калибрована бит-в-бит по канону H.e. (276 эф. дн = 9.2 эф. мес; R-скрипт 9.2×30)',
               'межрасовый размах A: 237.5 (гоблин) → 363.7 (Повелитель) эф. дн — физиологично',
               'B даёт орку (105 кг) те же 276 дней, что гоблину (20 кг) — биофизически невозможно'],
    flags=['ЭЛЬФ: канон Бестиария §2.7 = 10 эф.мес = 300 эф. дн vs модель A(52 кг) = 301.5 эф. дн '
           '→ Δ0.5% ✓ — см. ELF_PHYSIOLOGY.json v1.2 (ratio-тест); северный компактный подтип (28 кг) '
           '→ A = 258.3 эф. дн, канона НЕТ — ВОПРОС АВТОРУ',
           'ДЕМОНЫ: не углеродная биология (Бездна, время 1:1.3) — гестация не применима без модели',
           'ДРАКОНИТ: потомок Функции Мира — отдельная ветка (ядро/кладка?) — ВОПРОС'],
    демография=dict(население=156_524_634,
                    ...
                    беременных_одновременно=round(156_524_634 / 200 * 276 / (365.25 * 9 / 7 * 9 / 7))))  # 276 эф.дн / 603.78 эф.дн-цикл
ver = VERDICT_GEST['демография']
pchk('P-C10 рождений за цикл (стационарно)', '156524634 / 200', 156524634 / 200)
pchk('P-C11 одновременно беременных (гестация 276 эф.дн в цикле 603.78)',
...
out = {
    ...
    'gestation_table': {nm: dict(M_kg=g['M_kg'], A_ef_days=g['model_A_ef_days'],
...
print(f'  ВЕРДИКТ ГЕСТАЦИИ: модель A (аллометрия, эф. дн, якорь 276) ПРИНЯТА; B отклонена')
