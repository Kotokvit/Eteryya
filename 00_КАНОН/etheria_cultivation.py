...
VERDICT_GEST = dict(
    принят='МОДЕЛЬ A — аллометрическая: гестация = 280·(M/36.5)^0.25 ЗЕМНЫХ суток '
           '(эф. рендер ×7/9: H.e. 280 зем. д = 217.8 эф. сут = 7.16 эф. мес — «эффективных 7.1» канона)',
    ...
    rationale=['A калибрована бит-в-бит по канону H.e. (280 зем. д = 9.2 зем. мес; эф. рендер 217.8 сут)',
    ...
    flags=['ЭЛЬФ: канон Бестиария §2.7 = 10 мес = 304.4 ЗЕМ. д (236.8 эф. сут) vs модель A(52 кг) = 305.9 зем. д '
           '→ Δ0.5% ✓ — см. ELF_PHYSIOLOGY.json v1.1 (ratio-тест); северный компактный подтип (28 кг) '
           '→ A = 262 зем. д, канона НЕТ — ВОПРОС АВТОРУ',
    ...
    демография=dict(население=156_524_634,
                    ...
                    рождений_земной_год=round(156_524_634 / 200 * 7 / 9),  # 1 цикл = 9/7 зем. года (κ-календарь)
                    беременных_одновременно=round(156_524_634 / 200 * 280 * 7 / 9 / 365.25)))  # 217.8 эф.сут / 365.25 эф.сут-цикл
ver = VERDICT_GEST['демография']
pchk('P-C10 рождений за цикл (стационарно)', '156524634 / 200', 156524634 / 200)
pchk('P-C11 одновременно беременных (гестация 217.78 эф.сут в цикле 365.25)',
     '156524634 / 200 * 280 * 7 / 9 / 365.25', 156524634 / 200 * 280 * 7 / 9 / 365.25)
...
out = {
    ...
    'gestation_table': {nm: dict(M_kg=g['M_kg'], A_earth_days=g['model_A_days'],
                                 A_etherian_days=round(g['model_A_days'] * 7 / 9, 1),
                                 B_days=g['model_B_days'])
                        for nm, g in GEST.items()},
    'p3_bridge': BRIDGE,
