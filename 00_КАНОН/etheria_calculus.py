...
Аксиомы-входы (CANON-LOCKED):
  ...
  A6  T_rot        = 18.667 зем.ч     — период вращения (= 24 эт.ч = 7/9 зем. сут) [§8.4: восстановлено]
...
line()
eth_hour_h = kappa                   # эт. час = 7/9 зем. ч (истинная строка «24 ЭТ.ч = 18.67 ЗЕМ.ч»,
                                      #  свап эры-2 давал «24 зем.ч = 18.67 эт.ч» → 30.86 ч — §1.5/§8.4)
reg('eth_hour_in_earth_hours', eth_hour_h, '0.777778', tol=mpf('5e-7'), unit='ч (эт.час = κ зем.ч, 46 мин 40 с)')
...
reg('cycle_in_eth_days', Kf*YEAR_D/eth_day_earth_days, '603.78', tol=mpf('5e-4'), unit='эт.сут (≈ 20 мес×30 + 3.78 вис.)')
# 8 циклов ↔ земля
y8 = 8*Kf
m8 = (y8 - 10)*12
...
reg('Foucault_period_47N_h', T_fou_h, '25.47', tol=mpf('5e-3'), unit='зем.ч (= 32.75 эт.ч)')
...
print(f"    В этерианских минутах: {mp.nstr(rate_eth, 5)} °/эт.мин (канон «0.18°/эт.мин» ✓ — рамочно-инвариантно)")
...
print(f"    16 земных лет = {mp.nstr(years_et, 5)} эт. лет [канон «~12.4» ✓]")
print(f"    Возраст: земной 18 (2+16), эфический {mp.nstr(mpf(2)+years_et, 5)}, визуальный 90 (18×5.0)")
reg('twin_earth16_to_ef', years_et, '12.4', tol=mpf('5e-3'), unit='эт. лет')
